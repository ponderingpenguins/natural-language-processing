...
Table~\ref{tab:tokenizer_coverage} reports tokenizer statistics for the shared BPE tokenizer (same tokenizer across all runs). Although the table itself focuses on corpus size and split-rate summaries, the underlying coverage statistics show zero token-, type-, and document-level OOV on train, dev, and test. This is expected, because a BPE tokenizer decomposes unseen words into subword units rather than leaving them as true out-of-vocabulary items. In other words, coverage is complete, but full coverage does not mean zero fragmentation: words are still split into roughly 1.19 subword tokens on average. This matters for interpretation, because it suggests that the tokenizer is unlikely to hurt performance through unknown words, but it may still make some named entities and domain-specific terms harder to use consistently when they are broken into multiple pieces.
...
The World and Sports classes are generally easier to classify, with F1 scores above 90\% in most configurations. The Sci/Tech class is slightly harder, with F1 scores typically around 87--88\%. The most challenging class is Business, where F1 scores range from 86\% to 87\%, depending on the model and sequence length. One detail that is not fully visible from the heatmap alone is that the LSTM at seq128 has noticeably lower recall on the World class than the CNN at the same sequence length (89.63\% vs.\ 92.16\%). This suggests that, even when the overall macro-F1 values are very close, the CNN is somewhat better at recovering World examples at the main comparison length. A plausible explanation is that World articles often contain relatively local lexical cues, such as country names, political actors, and event words, which may be captured more reliably by the CNN.
